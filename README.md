...
게임의 최고의 플레이를 녹화하는 것을 잊었을 때, 작업 중 예상치 못한 오류로 저장하지 못한 작업물이 사라졌을 때, 온라인 강의 중 놓친 설명을 다시 확인하고 싶을 때, ReplayCapture는 당신에게 타임머신이 되어줍니다. 

ReplayCapture는 화면을 계속 임시 보관하다가 사용자가 버튼이나 단축키를 누르면 **방금까지의 모니터 화면 이력을 시스템 소리와 함께 MP4 파일로 저장**합니다. 

> ### 이런 분들을 위해 만들었습니다
>
> - 🎮 방금 지나간 게임 장면을 하이라이트만 손쉽게 남기고 싶은 사용자
> - 🧑‍💻 일상의 각종 작업 중 발생하는 예상치 못한 오류를 대처하고 싶은 사용자
> - 📚 강의나 회의에서 놓친 직전 설명을 다시 확인하고 싶은 사용자
...
| **저장 길이** | GUI 버튼으로 지금 한 번만 저장할 길이 | 기본값을 바꾸지 않고 이번만 30초 저장 |
...
| 녹화 멈추기 | 메인 화면의 **녹화 중지** |
| 완전히 종료하기 | 왼쪽 메뉴의 **종료** 또는 트레이 → **종료** |
| Windows 시작 시 실행 | **설정** 탭의 **Windows 로그인 시 앱 자동 실행** |

창의 `X` 버튼은 프로그램을 종료하지 않고 트레이로 숨깁니다. 녹화 중지, 화면 잠금, 절전 시에는 버퍼를 비웁니다. 소리 연결에 실패하면 영상만 녹화합니다. **정보** 메뉴에서 개발자·버전·사용권을 확인할 수 있습니다. 개인·비영리 목적 무료 사용이며 자세한 조건은 [사용권 계약](SOFTWARE_LICENSE.ko.md)을 참고하세요.
